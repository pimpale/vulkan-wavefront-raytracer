...
layout (set = 0, binding = 3) buffer DummyValues {
  uint dummy_values[];
};

layout (set = 0, binding = 4) buffer DummyValuesOut {
  uint dummy_values_out[];
};
...
uint dummy_use(uint always_zero) {
  ...
  return dummy_storage[0] + dummy_keys[0] + dummy_keys_out[0] + dummy_values[0] + dummy_values_out[0];
}
